...
welcomemsg() { \
	dialog --title "Welcome!" --msgbox "Manjaro Bootstrapping Script!\\n\\nThis script will automatically install a fully-featured Linux desktop, which I use as my main machine.\\n" 10 60
	}

selectdotfiles() { \
	edition="$(dialog --title "Select wm version." --menu "Select which version you wish to have as default:" 10 70 2 i3 "The classic version uses i3." dwm "This version of uses suckless's dwm." custom "If you are supplying commandline options." 3>&1 1>&2 2>&3 3>&1)" || error "User exited."
	}
...
gitmakeinstall() {
	dir=$(mktemp -d)
	dialog --title "Package Installation" --infobox "Installing \`$(basename "$1")\` ($n of $total) via \`git\` and \`make\`. $(basename "$1") $2" 5 70
...
dialog --title "Package Installation" --infobox "Installing \`basedevel\` and \`git\` for installing other software." 5 70
...
mkdir "/home/$name/Pictures/Screenshots"
...
newperms "%wheel ALL=(ALL) ALL #MINES
