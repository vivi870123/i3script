...
let g:lightline = {
	...
	\ 'colorscheme': 'hybrid',
	\ 'active': {
	\   'left': [ ['homemode'], ['filename_active'], ['blame'], ['cocstatus'], ['method']],
	\   'right':[['lineinfo'], ['gitinfo'], ['cocerror'],['cocwarn'],['cocfix'],['anzu']],
	\ },
	\ 'inactive': {
	\   'left': [['homemode'], ['filename_active']],
	\   'right':[['lineinfo'], ['gitinfo']],
	\ },
	...
	\ 'subseparator': { 'left': "|", 'right': "A"}
	\ }


function! s:lightline_is_lean() abort
  return &filetype =~? '\v^coc-explorer|diff|vista|magit|deol|undotree(diff)?$'
endfunction

function! s:lightline_is_plain() abort
  return &buftype ==? 'terminal' || &filetype =~? '\v^help|deol|gina|coc-explorer|codi|vista_kind$'
endfunction
...
function! LightlineFilenameActive() abort
  ...
  if &filetype ==? 'coc-explorer'
    return get(g:lightline, 'EXPLORER', '')
  endif
  if &filetype ==? 'tagbar'
